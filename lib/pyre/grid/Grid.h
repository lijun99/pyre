...
public:
    ...
    [[nodiscard]] PYRE_HOST_DEVICE constexpr auto operator[](const index_type & idx) const
        -> reference;
    // the cell at a given offset, same trust
    [[nodiscard]] PYRE_HOST_DEVICE constexpr auto operator[](difference_type off) const
        -> reference;
