...
//
// build against a pyre installed with cuda under {PREFIX}:
//   nvcc -std=c++20 -arch=native --expt-relaxed-constexpr \
//        -I$PREFIX/include -L$PREFIX/lib -Xlinker -rpath=$PREFIX/lib \
//        -lpyre -ljournal -o grid_stencil grid_stencil.cu
//
// {--expt-relaxed-constexpr} lets device code use the constexpr std::array machinery under
// pyre's indices and shapes
...
main()
{
    ...
    for (int i = 1; i < n - 1; ++i)
        for (int j = 1; j < n - 1; ++j)
            worst = std::fmax(worst, std::fabs(lap[{ i, j }] - 4.0));
    double edge =
        lap[{ 0, n / 2 }] + lap[{ n - 1, n / 2 }] + lap[{ n / 2, 0 }] + lap[{ n / 2, n - 1 }];
