...
const char * const pyre::extensions::cuda::vector::imul__doc__ = "in-place subtraction of two vectors";
...
pyre::extensions::cuda::vector::imul(PyObject *, PyObject * args) {
    ...
    switch(v1->dtype) {
    case PYCUDA_DOUBLE: //double
        cudalib::elementwise::imul<double>((double *)v1->data, other, v1->size);
        break;
    case PYCUDA_FLOAT: //single
        cudalib::elementwise::imul<float>((float *)v1->data, (float)other, v1->size);
        break;
    default:
        PyErr_SetString(PyExc_TypeError, "types other than float/double are not implenmented yet");
        return 0;
